...
# chia theo comment, merge theo comment luon
...
alot_com = na.omit(ana[(ana$X9.5...15.5.com / ana$X2.5...8.5.com )> 2 & ana$X9.5...15.5.com > 2 ,])
...
stable = alot_com[alot_com$X16.5...22.5.com / alot_com$X9.5...15.5.com > 0.8,]
almost_noChat = alot_com[alot_com$X16.5...22.5.com <= 1,]
slightly = alot_com[alot_com$X16.5...22.5.com / alot_com$X9.5...15.5.com <=0.8 & alot_com$X16.5...22.5.com > 1,]
...
for (i in 1:length(datalist))
{
  ...
  s_sum_eachday[i] = sum(s_merge_f[[i]][[32]], na.rm = T)
...
for (i in 1:length(datalist))
{
  ...
  n_sum_eachday[i] = sum(n_merge_f[[i]][[32]], na.rm = T)
...
for (i in 1:length(datalist))
{
  ...
  sli_sum_eachday[i] = sum(sli_merge_f[[i]][[32]], na.rm = T)
...
write.csv(data_3parts, "data_3parts_comment_merge_theo_comment.csv", row.names = F)


sort(table(stable$unknown), T)
plot(sort(table(stable$unknown), T))
