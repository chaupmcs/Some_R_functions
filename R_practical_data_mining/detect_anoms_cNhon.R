detect_anoms <- function(data, k = 0.49, alpha = 0.05, num_obs_per_period = NULL,
  ...
  if (length(rle(is.na(c(NA,data[[2L]],NA)))$values)>3){
    ## The 2 NAs used here on purpose to just delete the data when
    #  it has NAs on the middle.
    #  for instance, let's suppose we have a vector named "a":
    #     a=c(NA, NA, 1, 2, NA), we still keep "a" data, clean it by "na.omit" function.
    # another example: b=c(1, NA,2, 3, 4, 5), we reject b 'cuz it has NA value at 2nd element.
  ...
  for (i in 1L:max_outliers){
    ...
    temp_max_idx <- which(ares == R)[1L] #the first matching position 
    
    R_idx[i] <- data[[1L]][temp_max_idx] #store the value in vector R_idx
    
    data <- data[-which(data[[1L]] == R_idx[i]), ] #put the value away
    ...
    t <- qt(p,(n-i-1L)) # Find the p percentiles of the Student 
                      # t distribution with (n-i-1) degrees of freedom
